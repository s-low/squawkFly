...
class KFilter(object):
	...
	def __init__(self):
		...
		cv.SetIdentity(self.kf.process_noise_cov, cv.RealScalar(processNoiseCovariance))
		cv.SetIdentity(self.kf.measurement_noise_cov, cv.RealScalar(measurementNoiseCovariance))
		cv.SetIdentity(self.kf.error_cov_post, cv.RealScalar(errorCovariancePost))
