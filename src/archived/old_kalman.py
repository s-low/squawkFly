    ...
    for b_index, b in enumerate(f0["x"]):
        ...
        for b1_index, b1 in enumerate(f1["x"]):
            ...
            if sep < max_dist:
                ...
                cv.SetIdentity(
                    kf.process_noise_cov, cv.RealScalar(processNoiseCovariance))
                cv.SetIdentity(
                    kf.measurement_noise_cov, cv.RealScalar(measurementNoiseCovariance))
                cv.SetIdentity(
                    kf.error_cov_post, cv.RealScalar(errorCovariancePost))
